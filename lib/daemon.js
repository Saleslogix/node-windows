...
var daemon = function(config){
  ...
  Object.defineProperties(this,{
    ...
    stoptimeout: {
      ...
      writable: false,
      configurable: false,
      value: value: config.hasOwnProperty('stoptimeout') ? config.stoptimeout : 30
    },
    ...
    install: {
      ...
      value: function(dir){
        ...
        fs.exists(dir,function(exists){
          if (!exists){
            me.log.info('Creating service output directory: ' + dir);
            fs.mkdirSync(dir);
          }

          // Write the configuration file
          fs.writeFile(path.resolve(dir,me.id+'.xml'),me._xml,function(err){
            if (err) {
              me.log.error('Error creating service xml. Message: ' + err.message);
              throw err;
            }

            me.log.info('Service executable XML created.');

            // Write the exe file
            winsw.createExe(me.id,dir,function(err){
              if (err) {
                me.log.error('Error creating service exe. Message: ' + err.message);
                throw err;
              }

              me.log.info('Service executable created.');
              var installCmd = '"'+path.resolve(dir,me._exe)+'" install';
              var statusCmd = '"' + path.resolve(dir,me._exe) + '" status';
              me.execute(installCmd,function(){
                sleep(2);
                me.log.info('Install command on service exe finished.');
                me.log.info('Running status command: ' + statusCmd);
                exec(statusCmd, { encoding: 'utf-8' }, function(err, stdout) {
                  var status = stdout.trim();
                  if (status === 'NonExistent') {
                    me.log.error('Error executing service install. Status: ' + status);
                    sleep(2); // flush logs
                    throw new Error('Service exe was created but could not be installed.');
                  }
  
                  sleep(2);
                  me.log.info('Finished executing service install, emitting install event.');
                  me.emit('install');
                });
    ...
    start: {
      ...
      value: function(){
        var me = this;
        var nameRequiredMessage = "A name for the service is required.";
        var notFoundMessage = 'The service "'+this.name+'" does not exist or could not be found.';
        if (this.name == null){
          me.log.error(nameRequiredMessage);
          throw new Error(nameRequiredMessage);
        }

        if (!this.exists){
          me.log.error(notFoundMessage);
          ...
          if (err){
            if (err.code == 2){
              if (err.message.indexOf('already been started') >= 0 && err.message.indexOf('service name is invalid') < 0){
                ...
                me.log.error('Error running net start. Message: ' + err.message);
                me.emit('error',err);
                return;
              }
            } else {
        ...
        wincmd.isAdminUser(function(isAdmin){
          if (isAdmin) {
            ...
            if (me.user.account !== null && me.user.password !== null){
              ...
              me.log.info('Executing command using user/password: ' + _cmd);
              ...
              me.log.info('Executing command using sudo: ' + cmd);
              wincmd.sudo(cmd,me.sudo.password||'',options,callback);
            } else {
              me.log.info('Executing command using elevate: ' + cmd);
              wincmd.elevate(cmd,options, callback);
            }
          } else {
            me.log.error('Error executing command: ' + cmd);
            throw PermError;
          }
        });
      }
